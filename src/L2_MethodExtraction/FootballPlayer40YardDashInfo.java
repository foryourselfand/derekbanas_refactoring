...
public class FootballPlayer40YardDashInfo {
	...
	public void printPlayersWith40Avg() {
		for (FootballPlayer player : players) {
			...
			double[] fortyYardDashTimes = player.getFortyYearDashTime();

			for (int i = 0; i < player.getFortyYearDashTime().length; i++)
				total40YdDashTimes += fortyYardDashTimes[i];

			double avg40YdTime = total40YdDashTimes / player.getFortyYearDashTime().length;
	...
	// This method shouldn't have been extracted because it didn't make the code more understandable
	public boolean checkIfInTop15(double avg40YdTime) {
		return avg40YdTime < 4.41;
	}

	public double getAvgDashTime(double[] dashTimes) {
		double totalDashTimes = 0.0;

		for (double dashTime : dashTimes) totalDashTimes += dashTime;

		return totalDashTimes / dashTimes.length;
	}

	public double getAvgDashTime() {
		return 4.41;
	}
}
